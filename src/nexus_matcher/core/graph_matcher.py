...
class SchemaGraphBuilder:
    ...
    TYPE_GROUPS: ClassVar[dict[str, set[str]]] = {
    ...
    def build_graph(self, schema_name: str, fields: list[dict]) -> SchemaGraph:
        ...
        if not NETWORKX_AVAILABLE:
            raise ImportError(
                "networkx is required for graph-based matching. Install with: pip install networkx"
            )
    ...
    def _add_parent_child_edges(self, schema: SchemaGraph) -> None:
        ...
        for field_id, fld in schema.fields.items():
            if fld.parent_path and fld.parent_path in schema.fields:
                # Parent -> Child edge
    ...
    def _add_sibling_edges(self, schema: SchemaGraph) -> None:
        ...
        for field_id, fld in schema.fields.items():
            parent = fld.parent_path or "__root__"
    ...
    def _add_type_similarity_edges(self, schema: SchemaGraph) -> None:
        ...
        for field_id, fld in schema.fields.items():
            type_group = self._get_type_group(fld.data_type)
...
class GraphStructuralMatcher:
    ...
    def __init__(
        ...
        self._source_graph: SchemaGraph | None = None
        self._target_graph: SchemaGraph | None = None
    ...
    def _context_similarity(
        ...
        source_neighbors = self._get_neighbor_types(self._source_graph, source.path)
        target_neighbors = self._get_neighbor_types(self._target_graph, target.path)
    ...
    def get_graph_stats(self) -> dict:
        ...
        if self._source_graph:
            stats["source"] = {
                ...
                "max_depth": max((f.depth for f in self._source_graph.fields.values()), default=0),
            }
        ...
        if self._target_graph:
            stats["target"] = {
                ...
                "max_depth": max((f.depth for f in self._target_graph.fields.values()), default=0),
            }
...
class HybridMatcher:
    ...
    def rerank_with_structure(
        ...
        graph_scores = {r.target_field: r for r in graph_results}
        ...
        for field_id, semantic_score in semantic_results:
            ...
            if graph_result:
                ...
                combined_score = (
                    self.semantic_weight * semantic_score + self.graph_weight * graph_score
                )
                details = {
...
if __name__ == "__main__":
    ...
    for i, match in enumerate(matches, 1):
        ...
        print(
            f"   Structural: {match.structural_score:.4f}, Context: {match.context_score:.4f}, Type: {match.type_score:.4f}"
        )
