...
from ._fakes import make_entry, make_match
from ._legacy_console import run_on_legacy_codepage
...
# The matcher stub, the CLI-input files and the domain-object builders live in
# conftest.py; the legacy-code-page child process lives in _legacy_console.py. Both are
# shared with test_cli_json_governance.py, which exercises the same two hazards -- the
# encoding matrix and the non-TTY path -- against the JSON surface.


def _run_on_legacy_codepage(tmp_path, argv, encoding="cp437", columns="80", field_path=None):
    return run_on_legacy_codepage(
        tmp_path, argv, encoding=encoding, columns=columns, field_path=field_path
    )
...
def test_bracketed_business_name_reaches_the_table(tmp_path, cli_inputs, install_matcher):
    """
    ...
    """
    schema, dictionary = cli_inputs
    entry = make_entry(business_name="[deprecated] Customer Email")
    install_matcher({"customer.email": [make_match(entry=entry)]})
