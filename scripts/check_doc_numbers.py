...
carried the numbers from BEFORE the OMOP leakage fix -- the exact figures README's own
"Benchmark construction, and a leak we found in it" section retracts. Documents outside a
lane are RECORDED here rather than fixed, and listed in the lane's follow-ups.

**2026-08-11: 141 -> 72.** `docs/BENCHMARK_REGISTRY.md` was re-derived against its named
artifacts and its 69 entries were deleted, per the "fixing a document" rule below. Claims
verified went 110 -> 202. The remaining 72 belong to `CHANGELOG.md`,
`docs/ENHANCEMENT_JOURNEY.md`, `docs/PROJECT_STATE.md`, `docs/QUALITY_GATES.md`,
`docs/RESEARCH_ALIGNMENT.md`, `README.md` and four module docs, all still pre-fix. Nothing
was added: the removal was computed as `KNOWN_MISMATCHES & reproduced`, so a stale entry
could only leave this set by the document becoming right.
